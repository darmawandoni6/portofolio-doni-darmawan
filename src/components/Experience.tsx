      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-6 sm:space-y-8 max-w-4xl mx-auto">
          {WORK_EXPERIENCES.map((exp: ExperienceType, index: number) => (
            <div
              key={exp.company}
              className="glass-card glass-card-hover rounded-2xl p-5 sm:p-8 border border-slate-800/90 relative overflow-hidden group"
            >
              {/* Left Color Accent Bar */}
              <div
                className={`absolute top-0 left-0 bottom-0 w-1.5 ${
                  index === 0
                    ? 'bg-gradient-to-b from-cyan-400 to-indigo-600'
              ...
              <div className="space-y-5 sm:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-slate-800/80 pb-4">
                  <div>
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-xs font-semibold text-cyan-400">
                        {exp.type}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-wrap sm:flex-col sm:items-end text-xs font-mono text-slate-400 gap-3 sm:gap-1 pt-1 sm:pt-0 border-t sm:border-0 border-slate-800/60">
                ...
                <div className="space-y-3">
                  <h4 className="text-[11px] sm:text-xs uppercase tracking-wider text-slate-400 font-mono flex items-center gap-1.5">
                ...
                {/* Tech Stack Pills */}
                <div className="pt-4 border-t border-slate-800/60 flex items-center gap-2 flex-wrap">
                  ...
                  {exp.techStack.map((tech) => (
                    <span
                      key={tech}
                      className="px-2.5 py-1 rounded-lg bg-slate-900 text-xs font-mono text-slate-300 border border-slate-800"
                    >
                      {tech}
                    </span>
                  ))}
